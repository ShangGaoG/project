...
class HCAT(nn.Module):
    ...
    def __init__(self, backbone, featurefusion_network, num_classes):
        ...
        self.depth_wight = torch.nn.Parameter(torch.tensor([0.1]), requires_grad=True)
    ...
    def forward(self, search, template):
        ...
        src_search = torch.add(src_search_rgb, self.depth_wight*src_search_d)
        src_template = torch.add(src_template_rgb, self.depth_wight*src_template_d)
    ...
    def track(self, search):
        ...
        # tic = time.time()
        search_rgb, search_d = search[:,:3,:,:],search[:,3:,:,:]
        features_search_rgb = self.backbone.track(search_rgb)
        features_search_d = self.backbone.track(search_d)
        ...
        # src_template, mask_template = feature_template[-1].decompose()
        # assert mask_template is not None
        src_search = torch.add(features_search_rgb[-1],self.depth_wight* features_search_d[-1])
    ...
    def template(self, z, feature_size):
        ...
        self.zf = torch.add(zf_rgb[-1], self.depth_wight*zf_d[-1])
