...
from ltr.models.neck.feature_rgbd_rgbquery_search import build_featurefusion_network
########################
##   8.25 PPT Page2   ##
########################
class HCAT(nn.Module):
    ...
    def __init__(self, backbone, featurefusion_network, num_classes):
        ...
        self.query_embed_template = nn.Embedding(32, hidden_dim)
        self.query_embed_search = nn.Embedding(256, hidden_dim)
        ######GAO#######
    ...
    def forward(self, search, template):
        ...
        feature_search_rgb, pos_search_rgb = self.backbone(search_rgb)
        feature_template_rgb, pos_template_rgb = self.backbone(template_rgb)
        feature_search_d, pos_search_d = self.backbone(search_d)
        feature_template_d, pos_template_d = self.backbone(template_d)

        assert pos_search_rgb[-1].equal(pos_search_d[-1]), pos_template_rgb[-1].equal(pos_template_d[-1])
        ...
        # src_search_rgb, src_search_d = feature_search_rgb[-1], feature_search_d[-1]
        # src_template_rgb, src_template_d = feature_template_rgb[-1], feature_template_d[-1]
        # src_search = torch.cat((src_search_rgb, src_search_d),dim=1)
        # src_template = torch.cat((src_template_rgb, src_template_d),dim=1)



        hs = self.featurefusion_network(self.input_proj(feature_template_rgb[-1]),
                                        self.input_proj(feature_search_rgb[-1]),
                                        self.input_proj(feature_template_d[-1]),
                                        self.input_proj(feature_search_d[-1]),
                                        pos_template_rgb[-1], pos_search_rgb[-1],
                                        self.query_embed_template.weight,
                                        feature_search_rgb[-1])
    ...
    def track(self, search):
        ...
        # tic = time.time()
        search_rgb, search_d = search[:,:3,:,:],search[:,3:,:,:]
        feature_search_rgb = self.backbone.track(search_rgb)
        feature_search_d = self.backbone.track(search_d)

        # print('backbone_time'+str(time.time()-tic))
        pos_search = self.pos_search
        feature_template_rgb = self.zf_rgb
        feature_template_d = self.zf_d
    ...
    def template(self, z, feature_size, draw=True):
        z_rgb, z_d = z[:,:3,:,:],z[:,3:,:,:]
    
        zf_rgb, pos_template = self.backbone(z_rgb)
        zf_d, pos_template_d = self.backbone(z_d)
        b, c, _, _ = zf_rgb[-1].shape
        src_search = torch.ones([b, c, feature_size, feature_size]).to(zf_rgb[-1].device)
        pos_search = []
        pos_search.append(self.backbone[1](src_search).to(zf_rgb[-1].dtype))
        self.zf_rgb = zf_rgb
        self.zf_d = zf_d
