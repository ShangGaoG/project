...
savepath='/ssd-nvme1/gs/HCAT/feature_vis/resnet18_rgb'
def draw_features(width,height,x, savename):
    ...
    for i in range(width*height):
        ...
        pmin = np.min(img)
        pmax = np.max(img)
        img = ((img - pmin) / (pmax - pmin + 0.000001))*255  #float在[0，1]之间
...
class ResNet(Backbone):
    ...
    def forward(self, x, output_layers=None, draw=False):
        ...
        if draw:
            ...
            draw_features(8,8,x_.cpu().detach().numpy(),"{}/f1_conv1.png".format(savepath))
            x_=x = self.bn1(x)
            draw_features(8, 8, x_.cpu().detach().numpy(),"{}/f2_bn1.png".format(savepath))
            x_=x = self.relu(x)
            draw_features(8, 8, x_.cpu().detach().numpy(), "{}/f3_relu.png".format(savepath))
            if self._add_output_and_check('conv1', x, outputs, output_layers):
                return outputs

            x_=x = self.maxpool(x)
            draw_features(8, 8, x_.cpu().detach().numpy(), "{}/f4_maxpool.png".format(savepath))

            x_=x = self.layer1(x)
            draw_features(8, 8, x_.cpu().detach().numpy(), "{}/f5_layer1.png".format(savepath))

            if self._add_output_and_check('layer1', x, outputs, output_layers):
                return outputs

            x_=x = self.layer2(x)
            draw_features(8, 16, x_.cpu().detach().numpy(), "{}/f6_layer2.png".format(savepath))
            if self._add_output_and_check('layer2', x, outputs, output_layers):
                return outputs


            x_=x = self.layer3(x)
            draw_features(16, 16, x_.cpu().detach().numpy(), "{}/f7_layer3.png".format(savepath))
