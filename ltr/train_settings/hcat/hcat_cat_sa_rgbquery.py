...
import ltr.models.tracking.hcat_cat_sa_rgbquery as hcat_cat_sa_rgbquery_models
...
def run(settings):
    ...
    #Save 
    settings.exname = 'rgb3d-hcat_cat_sa_rgbquery'
    ...
    depthtrack_val = DepthTrack(root=settings.env.depthtrack_dir, split='test', dtype=input_dtype)
    ...
    model = hcat_cat_sa_rgbquery_models.hcat(settings)

    # Wrap the network for multi GPU training
    if settings.multi_gpu:
        model = MultiGPU(model, dim=0)

    objective = hcat_cat_sa_rgbquery_models.hcat_loss(settings)
