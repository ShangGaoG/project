...
import ltr.models.tracking.hcat_early_fusion as hcat_early
...
def run(settings):
    ...
    settings.M = 0.1
    settings.epoch = -1
    settings.exname = 'colormap-early-fusion'
    ...
    model = hcat_early.hcat(settings)

    # Wrap the network for multi GPU training
    if settings.multi_gpu:
        model = MultiGPU(model, dim=0)

    objective = hcat_early.hcat_loss(settings)
    ...
    trainer.train(151, load_latest=True, fail_safe=True)
