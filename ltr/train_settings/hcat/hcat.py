...
from ltr.dataset import Lasot, MSCOCOSeq, Got10k, TrackingNet, DepthTrack
import ltr.models.tracking.hcat as hcat_models
import ltr.models.tracking.hcat_sp_rgbd as hcat_sp_rgbd_models
import ltr.models.tracking.hcat_rgbd_add as hcat_rgbd_add_models
import ltr.models.tracking.hcat_early_fusion as hcat_fusion
...
def run(settings):
    ...
    dataset_train = sampler.HCATSampler([depthtrack_train], [1],
                                samples_per_epoch=50000, max_gap=100, processing=data_processing_train)
    
    dataset_val = sampler.HCATSampler([depthtrack_val], [1],
                                samples_per_epoch=50000, max_gap=100, processing=data_processing_val)
    ...
    model = hcat_fusion.hcat(settings)

    # Wrap the network for multi GPU training
    if settings.multi_gpu:
        model = MultiGPU(model, dim=0)

    objective = hcat_rgbd_add_models.hcat_loss(settings)
